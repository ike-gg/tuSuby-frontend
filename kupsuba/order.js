...
document.querySelectorAll(".orderButtonRow button").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    document
      .querySelectorAll(`div[data-input="${dataInput}"] button`)
      .forEach((buttonInside) => {
    ...
    if (dataInput == "subTier") {
      ...
      if (subTier == 2 || subTier == 3) {
        document
          .querySelectorAll('div[data-input="subDuration"] button')
          .forEach((button) => {
            button.style.display = "none";
            button.classList.remove("activeButtonOrder");
          });
        document.querySelector(
          'div[data-input="subDuration"] button'
        ).style.display = "block";
      } else {
        document
          .querySelectorAll('div[data-input="subDuration"] button')
